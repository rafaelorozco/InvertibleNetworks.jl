...
function forward(X::AbstractArray{Float32, 4}, J, D, CS::AffineCouplingLayerSLIM)
...
function inverse(Y::AbstractArray{Float32, 4}, J, D, CS::AffineCouplingLayerSLIM; save=false)
...
function backward(ΔY::AbstractArray{Float32, 4}, Y::AbstractArray{Float32, 4}, J, D, CS::AffineCouplingLayerSLIM; permute=false)

    # Recompute forward states
    X, X1_, X2_, gn, gs, S  = inverse(Y, J, D, CS; save=true)
    nx, ny, n_s, batchsize = size(Y)
